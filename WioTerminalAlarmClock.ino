...
#include "SAMCrashMonitor.h" // see https://github.com/cyrusbuilt/SAMCrashMonitor
...
void setup() {
  ...
  updateDelay.start(50);
  tft.fillScreen(TFT_BLACK);

  // Start the watchdog: *Note - 8 seconds seems to be the maximum through experimentation (SE)
  int watchdogTimeout = SAMCrashMonitor::enableWatchdog(8000);
...
bool synchRTCtoNTPtime(const char* address) {
  ...
  Serial.println("Synching RTC to NTP time begun");
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    if (!gotPacket) {
      Serial.println("UDP NTP packet response time out");
      // were not able to parse the udp packet successfully
      // clear down the udp connection
      udp.stop();
      Serial.println("Synch of RTC to NTP FAILED");
      return false;
    } else {
      Serial.println("UDP NTP response packet received");
      ...
      Serial.println("Synch of RTC to NTP SUCCESS");
      return true;
    }
  }
}
